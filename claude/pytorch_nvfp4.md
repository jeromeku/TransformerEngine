...
Frame 1: Python Entry Point - torch/nn/functional.py:6691-6772
Inputs:
...
Logic: The function acts as a convenience wrapper that:
Normalizes inputs using expand_single_value() - converts single values or None to lists
Converts enums to integers using enum_list_as_int_list() because native_functions.yaml can't pass enum types directly
Routes to native op torch._scaled_mm_v2
# Lines 6725-6756
...
Frame 2: ATen Dispatcher - aten/src/ATen/native/native_functions.yaml:7293-7296
Logic: PyTorch's code generation system reads native_functions.yaml and auto-generates Python bindings for torch._scaled_mm_v2. The YAML file specifies:
Function signature
...
Frame 3: CUDA Entry Point (Wrapper) - aten/src/ATen/native/cuda/ScaledBlas.cpp:1474-1498
Logic: Simple wrapper that allocates output tensor and calls the _out variant:
...
Frame 4: Core Implementation - aten/src/ATen/native/cuda/ScaledBlas.cpp:1279-1471
Logic: This is the main orchestration function that:
Validates inputs (device, shapes, dtypes)
Selects implementation from dispatch table
...
The check_nvfp4_recipe function (CUDAScaledBlas.cpp:125-148) validates:
...
Frame 5: NVFP4 Handler - aten/src/ATen/native/cuda/ScaledBlas.cpp:1209-1254
Logic: Prepares two-level scaling and validates NVFP4-specific requirements:
...
Frame 6: Scaled GEMM Orchestrator - aten/src/ATen/native/cuda/ScaledBlas.cpp:384-446
Logic: Prepares cuBLAS arguments and routes to the appropriate backend (cuBLASLt or TunableOp):
Tensor& _scaled_gemm(
...
Frame 7: cuBLASLt Wrapper - aten/src/ATen/cuda/CUDABlas.cpp:1967-2213
Logic: Configures cuBLASLt descriptors and dispatches to the CUDA kernel:
...
What Happens: cuBLASLt selects and launches a CUDA kernel based on:
...
Decode FP4 → FP8/FP16 using BlockWise1x16 scales:
...
Multiply global scales (alpha parameter) during accumulation
Accumulate in FP32 using tensor cores
Add bias (if provided) during epilogue
Convert FP32 → BF16 and store to output
Actual Library: The kernel is part of NVIDIA cuBLASLt (closed-source), specifically the FP4 GEMM implementation added in CUDA 12.8+. Key Performance Details:
