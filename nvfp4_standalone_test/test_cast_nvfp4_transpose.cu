...
enum ActivationType { Identity, GeLU, SiLU, ReLU, QGeLU, SReLU };
...
std::vector<InputType> create_transpose(const InputType* const input, const size_t rows,
                                        size_t cols) {
...
template <typename InputType>
void quantize_nvfp4_1d(float (*OP)(const float), const InputType* const input,
                       fp4e2m1x2* const output, fp8e4m3* const scales, const size_t rows,
                       const size_t cols, const size_t scales_stride, const float global_amax) {
...
template <typename InputType>
void compute_2d_mathematical_scales(float (*OP)(const float), const InputType* const input,
                                    const size_t rows, const size_t cols, const float global_amax,
...
template <typename InputType>
void quantize_nvfp4_2d(float (*OP)(const float), const InputType* const input,
                       fp4e2m1x2* const output, fp8e4m3* const scales, const size_t rows,
                       const size_t cols, const size_t scales_stride, const float global_amax) {
            ...
            for (size_t i = i_min; i < i_max; ++i) {
                for (size_t j = j_min; j < j_max; j += 2) {
                    ...
                    const float cached_y = ((j + 1) < j_max && cache_idx_x2 < block_size_X)
                                               ? cache_buffer[cache_idx_y][cache_idx_x2]
                                               : 0.0f;
...
template <typename InputType>
void quantize_nvfp4(float (*OP)(const float), const InputType* const input, fp4e2m1x2* const output,
                    fp8e4m3* const scales, const size_t rows, const size_t cols,
                    const size_t scales_stride, const float global_amax,
...
template <typename InputType>
void compute_ref(float (*OP)(const float), const InputType* input, fp4e2m1x2* output,
                 fp4e2m1x2* output_t, fp8e4m3* scales, fp8e4m3* scales_t, const float global_amax,
                 const size_t rows, const size_t cols, const size_t scales_stride,
                 const size_t scales_stride_t, const bool use_2d_quantization = false) {
    ...
    if (use_2d_quantization) {
        ...
        for (size_t i = 0; i < cols; ++i) {  // cols = 128, which becomes rows of transposed data
            const size_t block_X_orig =
                i / block_size_X;  // i was column index in original, so maps to block_X
            for (size_t block_Y_new = 0; block_Y_new < blocks_Y;
                 ++block_Y_new) {  // block in transposed coordinate
        ...
        quantize_nvfp4_2d(OP, input, output, nullptr, rows, cols, scales_stride,
                          global_amax);  // scales already filled
        quantize_nvfp4_2d(OP, input_t.data(), output_t, nullptr, cols, rows, scales_stride_t,
                          global_amax);  // scales_t already filled

    } else {
        quantize_nvfp4(OP, input, output, scales, rows, cols, scales_stride, global_amax,
                       use_2d_quantization);
        quantize_nvfp4(OP, input_t.data(), output_t, scales_t, cols, rows, scales_stride_t,
                       global_amax, use_2d_quantization);
    }
}

void compare_nvfp4_tensors(const std::string& name, const fp4e2m1* test_data,
                           const fp4e2m1* ref_data, const int rows, const int cols,
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; j += 2) {
            ...
            double2 test_data_pair =
                cvt_fp4x2_to_double2(*reinterpret_cast<const fp4e2m1x2*>(&test_data[idx / 2]));
            double2 ref_data_pair =
                cvt_fp4x2_to_double2(*reinterpret_cast<const fp4e2m1x2*>(&ref_data[idx / 2]));
            ...
            for (int k = 0; k < 2; ++k) {
                ...
                if (assertion) {
                    total_mismatches++;
                    std::string msg =
                        "Mismatch at place (" + std::to_string(idx + k) +
    ...
    if (total_mismatches > 0) {
        ...
        std::cout << "Mismatch rate: " << (100.0 * total_mismatches) / (rows * cols) << "%"
                  << std::endl;
        if (mismatch_messages.size() > 100) {
            std::cout << "... and " << (mismatch_messages.size() - 100)
                      << " more mismatches (showing first 100)" << std::endl;
        }
        std::cout << "============================" << std::endl;

        throw std::runtime_error("Found " + std::to_string(total_mismatches) +
                                 " mismatches in tensor " + name);
    } else {
...
void dump_nvfp4_tensor_data(const std::string& prefix, const fp4e2m1* test_data,
                            const fp4e2m1* ref_data, const int rows, const int cols) {
    ...
    if (test_out.is_open() && ref_out.is_open() && diff_out.is_open()) {
        ...
        std::cout << "DEBUG: Dumped tensor data to files: " << test_file << ", " << ref_file << ", "
                  << diff_file << std::endl;
    } else {
...
void print_detailed_tensor_comparison(const std::string& name, const fp4e2m1* test_data,
                                      const fp4e2m1* ref_data, const int rows, const int cols) {
    ...
    for (int i = 0; i < std::min(check_count, total_elements); ++i) {
        double2 test_pair =
            cvt_fp4x2_to_double2(*reinterpret_cast<const fp4e2m1x2*>(&test_data[i / 2]));
        double2 ref_pair =
            cvt_fp4x2_to_double2(*reinterpret_cast<const fp4e2m1x2*>(&ref_data[i / 2]));
...
void compareResults_nvfp4(const Tensor& test, const void* ref, const void* ref_t, const int rows,
                          const int cols, double atol = 1e-5, double rtol = 1e-8,
                          bool if_on_gpus = true, bool dump_data = false) {
...
template <typename InputType>
void performTest(float (*OP)(const float), const std::vector<size_t>& shape) {
    using namespace test;
    ...
    compute_ref<InputType>(OP, input.rowwise_cpu_dptr<InputType>(), ref_output.get(),
                           ref_output_t.get(), ref_scales.get(), ref_scales_t.get(), output.scale(),
                           rows, cols, scales_stride, scales_stride_t, use_2d_quantization);
    ...
    compare_scaling_factors<fp8e4m3>("scales", output.rowwise_cpu_scale_inv_ptr<fp8e4m3>(),
                                     ref_scales.get(), unpadded_blocks_Y, unpadded_blocks_X,
                                     scales_stride, scale_mismatches_num);
...
std::vector<std::vector<size_t>> tensor_dims = {
    {32, 32},      {32, 64},        {64, 32},      {64, 96},      {128, 128},  {256, 256},
    {512, 512},    {1024, 1024},    {2048, 2048},  {128, 256},    {8192, 128}, {2048, 160},
    {8, 32, 1024}, {16, 8, 4, 512}, {1024, 16384}, {4096, 13312},
};

// Only the Identity activation is currently supported.
std::vector<ActivationType> Activation_types = {ActivationType::Identity};

}  // namespace

static void usage(const char* argv0) {
    std::cerr << "Usage: " << argv0 << " [--shape 128x128] [--dtype bf16] [--activation identity]\n"
              << "  --shape: dims separated by 'x' (e.g. 128x128 or 16x8x4x512)\n"
              << "  --dtype: bf16|fp16|fp32\n"
              << "  --activation: identity|gelu|silu|relu|qgelu|srelu\n";
}
...
static std::vector<size_t> parse_shape(const std::string& s) {
    ...
    for (const char c : s) {
        if (c == 'x' || c == 'X' || c == ',' || c == ';') {
            if (!cur.empty()) {
                dims.push_back(static_cast<size_t>(std::stoull(cur)));
                cur.clear();
            }
            continue;
        }
    ...
    if (dims.size() < 2) {
        throw std::runtime_error("Shape must have at least 2 dims (e.g. 128x128)");
    }
    return dims;
}

static transformer_engine::DType parse_dtype(const std::string& s) {
    if (s == "bf16" || s == "bfloat16") return transformer_engine::DType::kBFloat16;
    if (s == "fp16" || s == "float16") return transformer_engine::DType::kFloat16;
    if (s == "fp32" || s == "float32") return transformer_engine::DType::kFloat32;
    throw std::runtime_error("Invalid dtype: " + s);
}

static ActivationType parse_activation(const std::string& s) {
    if (s == "identity" || s == "cast_only") return ActivationType::Identity;
    if (s == "gelu") return ActivationType::GeLU;
    if (s == "silu") return ActivationType::SiLU;
    if (s == "relu") return ActivationType::ReLU;
    if (s == "qgelu") return ActivationType::QGeLU;
    if (s == "srelu") return ActivationType::SReLU;
    throw std::runtime_error("Invalid activation: " + s);
}

int main(int argc, char** argv) {
    std::vector<size_t> shape = {128, 512};
    transformer_engine::DType input_type = transformer_engine::DType::kBFloat16;
    ActivationType act_type = ActivationType::Identity;
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if ((arg == "--shape" || arg == "-s") && i + 1 < argc) {
            shape = parse_shape(argv[++i]);
            continue;
        }
        if ((arg == "--dtype" || arg == "-d") && i + 1 < argc) {
            input_type = parse_dtype(argv[++i]);
            continue;
        }
        if ((arg == "--activation" || arg == "-a") && i + 1 < argc) {
            act_type = parse_activation(argv[++i]);
            continue;
        }
        throw std::runtime_error("Unknown/invalid arg: " + arg);
    }

    auto OP = &test::identity;
    std::cout << "Shape: " << shape[0] << " x " << shape[1] << std::endl;

    TRANSFORMER_ENGINE_TYPE_SWITCH_FP16_FP32_ONLY(input_type, InputType,
                                                  performTest<InputType>(OP, shape););
